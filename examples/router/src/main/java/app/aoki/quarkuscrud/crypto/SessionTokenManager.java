 ...
 *
 * <p>Session tokens are single-use, time-limited tokens that allow a controller to upgrade from
 * REST API to WebSocket connection securely.
 */
@ApplicationScoped
public class SessionTokenManager {
  ...
  public String generateToken(String sessionId) {
    ...
    SessionInfo info =
        new SessionInfo(sessionId, Instant.now().plus(TOKEN_VALIDITY_MINUTES, ChronoUnit.MINUTES));
   ...
   *
   * <p>Tokens are single-use - once validated, they are removed and cannot be reused.
   *
   ...
   *
   * <p>Should be called by a scheduled task.
   */
