/**
 * RouterServerTransport - ServerTransport implementation for router communication
 * Implements ServerTransport interface from jsapdu-over-ip library
 */
...
/**
 * ServerTransport implementation that connects to router via WebSocket
 */
export class RouterServerTransport implements ServerTransport {
   ...
   * Called by SmartCardPlatformAdapter
   */
  ...
  /**
   * Send event to router (forwarded to controllers)
   */
  emitEvent(event: RpcEvent): void {
    ...
    const message: RouterMessage = {
      type: "rpc-response", // Events piggyback on response channel
      data: event,
    };
